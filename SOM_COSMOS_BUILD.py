...
coorC20 = SkyCoord(ra = data['ALPHA_J2000'].data*u.degree, dec = data['DELTA_J2000'].data*u.degree)
idx, d2d, _ = match_coordinates_sky(coorIR,coorC20)
sep_limit = d2d < 1.*u.arcsec
idx = idx[sep_limit]
idy = sep_limit
# Keep IR galaxies with logM*>10  #REMOVED in v4d
mass_cut = data['lp_mass_med'][idx].data > 0.
...
irc[idx[mass_cut]] = True  # we'll use only M*>10 for calibration (due tocompleteness reasons)
...
# Training
somsp = sompy.SOMFactory.build(datin, mapsize=(dsz0,dsz1), mapshape='planar',lattice='rect',initialization='pca')
somsp.train(n_job=4,shared_memory='no')
# extract coordinates X and Y on the SOM grid, and BMU
ac = somsp.bmu_ind_to_xy(somsp.find_bmu(somsp._data)[0])
data["SOM_X"] = ac.T[0]; data["SOM_Y"] = ac.T[1]; data["SOM_BMU"] = ac.T[2]

# SOM keys for HDF5
lmax = len(max(filtused,key=len))*2+1
comp_list = np.array([i[0]+'-'+i[1] for i in col_list],dtype='U'+str(lmax))[np.newaxis,:]
#! comp_list = np.append(comp_list, 'HSC_i')
somsp._component_names = comp_list
...
# Redshift estimates 
cellmed_z,cellvar_z = som_utils.paint_cell(somsp,data['SOM_BMU'].data,data['lp_zBEST'].data)
data['z_SOM'] = som_predict.predict_from_nn(somsp, cellmed_z, datin, log=False, scale=None, nn=5)
# Stellar mass estimates
mass_nrm = data['lp_mass_med'] + 0.4*del_i # renormalized stellar mass
...
cellmed_sfr,cellvar_sfr = som_utils.paint_cell(somsp,data['SOM_BMU'][irc].data,sfrIR_nrm,mincount=2)
...
# formats={}
data.write(FOLDOUT+"som2020_cosmos2020_{}.fits".format(version),overwrite=True)

#HDF5 
somsp.id = data['ID'].data
print(cellmed_sfr)
somsp.__dict__['_dlabelz'] = cellmed_z
somsp.__dict__['_dlabelsfr'] = cellmed_sfr
somsp.__dict__['_dlabelmass'] = cellmed_mass
somsp.__dict__['_dlabelqf'] = cellQF
OUT = OutputToHDF5("som_{}x{}_{}.hdf5".format(dsz0,dsz1,version))
OUT.writeSOM(somsp)
OUT.close()

print("COSMOS part completed")
